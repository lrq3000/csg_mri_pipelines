function ImCalc_job_white()

root_path = 'D:\Stephen\DataToPreproc\ESA_Cosmonauts\Cosmo_nifti-new\tennis-test-data_reorg';
    ...
    for s=1:length(subjects{g}.names)
        ...
        for sessid=1:length(sessions)
            ...
            session = struct('id', sessions{sessid}, ...
                             'files', struct('white', [])); % Note: NEVER init with {}, always with [] because else the struct will be considered empty and unmodifiable! (famous errors: ??? A dot name structure assignment is illegal when the structure is empty. or ??? Error using ==> end)

            % Get full filepaths of all images
            whitepath = getimgpath(sesspath, 'struct');
            % Save the structural images
            session.files.white = check_exist(regex_files(whitepath, ['^s8m0wrp2.*\.(img|nii)$']));
...
% == Calculate difference between two sessions of white test for each subject
job = 0;
...
for g=1:length(groups)
    for s=1:length(subjects{g}.names)
        ...
        matlabbatch{job}.spm.util.imcalc.input = {
                                                data.groups{g}.subjects{s}.sessions{1}.files.white
                                                data.groups{g}.subjects{s}.sessions{2}.files.white
                                                };
        matlabbatch{job}.spm.util.imcalc.output = 'white_diff.nii'; % name the the output image
