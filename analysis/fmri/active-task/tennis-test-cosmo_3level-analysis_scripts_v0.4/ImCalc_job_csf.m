    ...
    for s=1:length(subjects{g}.names)
        ...
        for sessid=1:length(sessions)
            ...
            session = struct('id', sessions{sessid}, ...
                             'files', struct('csf', [])); % Note: NEVER init with {}, always with [] because else the struct will be considered empty and unmodifiable! (famous errors: ??? A dot name structure assignment is illegal when the structure is empty. or ??? Error using ==> end)

            % Get full filepaths of all images
            csfpath = getimgpath(sesspath, 'struct');
            % Save the structural images
            session.files.csf = check_exist(regex_files(csfpath, ['^s8m0wrp3.*\.(img|nii)$']));
...
% == Calculate difference between two sessions of csf test for each subject
job = 0;
...
for g=1:length(groups)
    for s=1:length(subjects{g}.names)
        ...
        matlabbatch{job}.spm.util.imcalc.input = {
                                                data.groups{g}.subjects{s}.sessions{1}.files.csf
                                                data.groups{g}.subjects{s}.sessions{2}.files.csf
                                                };
        matlabbatch{job}.spm.util.imcalc.output = 'csf_diff.nii'; % name the the output image
