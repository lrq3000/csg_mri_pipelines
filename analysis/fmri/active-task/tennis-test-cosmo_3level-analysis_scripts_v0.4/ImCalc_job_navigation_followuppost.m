function ImCalc_job_navigation_followuppost()
    ...
    for s=1:length(subjects{g}.names)
        ...
        for sessid=1:length(sessions)
            ...
            session = struct('id', sessions{sessid}, ...
                             'files', struct('navigation', [])); % Note: NEVER init with {}, always with [] because else the struct will be considered empty and unmodifiable! (famous errors: ??? A dot name structure assignment is illegal when the structure is empty. or ??? Error using ==> end)

            % Get full filepaths of all images
            navigationpath = getimgpath(sesspath, 'navigation');
            % Save the structural images
            session.files.navigation = check_exist(regex_files(navigationpath, ['^con_0001\.(img|nii)$']));
...
% == Calculate difference between two sessions of navigation test for each subject
job = 0;
    ...
    for s=1:length(subjects{g}.names)
        if numel(data.groups{g}.subjects{s}.sessions) >= 3  % only process subjects where there is a follow up (a 3rd session)
            ...
            matlabbatch{job}.spm.util.imcalc.input = {
                                                    data.groups{g}.subjects{s}.sessions{2}.files.navigation
                                                    data.groups{g}.subjects{s}.sessions{3}.files.navigation
                                                    };
            matlabbatch{job}.spm.util.imcalc.output = 'navigation_diff_followup-post.nii'; % name the the output image
...
% This eases access to the various images types
    ...
    switch imtype
        case 'navigation'
            fpath = fullfile(dirpath, 'navigation', 'Classical_ana');
