    ...
    for s=1:length(subjects{g}.names)
        ...
        for sessid=1:length(sessions)
            ...
            session = struct('id', sessions{sessid}, ...
                             'files', struct('grey', [])); % Note: NEVER init with {}, always with [] because else the struct will be considered empty and unmodifiable! (famous errors: ??? A dot name structure assignment is illegal when the structure is empty. or ??? Error using ==> end)

            % Get full filepaths of all images
            greypath = getimgpath(sesspath, 'struct');
            % Save the structural images
            session.files.grey = check_exist(regex_files(greypath, ['^s8m0wrp1.*\.(img|nii)$']));
...
% == Calculate difference between two sessions of grey test for each subject
job = 0;
...
for g=1:length(groups)
    for s=1:length(subjects{g}.names)
        ...
        matlabbatch{job}.spm.util.imcalc.input = {
                                                data.groups{g}.subjects{s}.sessions{1}.files.grey
                                                data.groups{g}.subjects{s}.sessions{2}.files.grey
                                                };
        matlabbatch{job}.spm.util.imcalc.output = 'grey_diff.nii'; % name the the output image
